...
morton_t zOrder(const Point &coord, int i) {
    if (coord.x < 0 || coord.x >= (1u << NBITS_PER_DIM))
        throw std::runtime_error("098245490432590890");
    if (coord.y < 0 || coord.y >= (1u << NBITS_PER_DIM))
        throw std::runtime_error("432764328764237823");
    const unsigned int x = static_cast<unsigned int>(coord.x);
    const unsigned int y = static_cast<unsigned int>(coord.y);

    morton_t morton_code = spreadBits(x) | (spreadBits(y) << 1u);

    // augmentation
    return (morton_code << 32u) | static_cast<unsigned int>(i);
}
    ...
    while (stack_size) {
        ...
        for (int i_child : {node.child_left, node.child_right}) {
            ...
            if (!child.bbox.contains(x0, y0) && barnesHutCondition(x0, y0, child)) {
                ...
                const float dr2 = std::max(100.f, dx * dx + dy * dy);
